...
const RULE2_BEFORE_MS = 60_000; // drip may start up to 60 s before its parent
 ...
 *   2. heuristic — same cwd and [start − 60 s, end + 10 min] ⊇ drip start,
 *      latest such host start winning, whatever its harness;
 *   3. otherwise the drip session stays a top-level row of its group.
 */
export function buildIndex(entries: SessionEntry[]): ProjectGroup[] {
  const hosts = entries.filter((e) => isHostKind(e.kind));
  const drip = entries.filter((e) => e.kind === 'drip');

  // Every session becomes a node exactly once; parents are linked after.
  const nodes = new Map<string, SessionNode>();
  for (const entry of [...hosts, ...drip]) {
    nodes.set(nodeKey(entry), { entry, children: [] });
  }
  const children = new Set<string>();
  for (const entry of drip) attachToParent(nodes, children, entry, hosts);

  const slugCwd = slugMap(entries);

  // Group every node by its cwd's worktree; drip children stay with their
...
/** drip's project slug for a cwd: `/` becomes `-` and `.` is dropped. */
export function dripSlug(cwd: string): string {
  return cwd.replace(/\./g, '').replace(/\//g, '-');
}

/**
 * slug → cwd for every cwd we know, under both slug dialects, so entries
 * that lack a cwd (old drip sessions without session.json, orphaned
...
function slugMap(entries: SessionEntry[]): Map<string, string> {
  ...
  for (const e of entries) {
    ...
    for (const slug of [e.slug, claudeSlug(e.cwd), dripSlug(e.cwd)]) if (!out.has(slug)) out.set(slug, e.cwd);
  }
  return out;
}
 ...
 * drip start — a long-running host session that began hours earlier still
 * qualifies).
 */
function attachToParent(
  ...
  // Rule 2: keep the candidate with the latest start ≤ drip start; a parent
